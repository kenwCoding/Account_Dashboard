...
function LoginForm({handleFormChange}: {handleFormChange: (formType: FormType) => void}) {
  const [isFormValid, setIsFormValid] = useState<boolean | null>(null);

  const { register, handleSubmit, watch } = useForm<LoginFormSchema>({
  ...
  return (
    ...
    >
      <div className='mx-auto'>
        <h1 className='text-xl font-extrabold uppercase'>Login</h1>
      </div>
      <LoginFormField
        label="username"
        name="username"
      ...
      <div className='flex justify-between items-center'>
        <Button
          ...
          `}
          type="button"
          onPress={() => handleFormChange(FormType.REGISTER)}
        >
...
type LoginFormFieldProps = z.infer<typeof LoginFormFieldSchema>

const LoginFormField = (props: LoginFormFieldProps) => {
  const { label, type, inputRegister } = props;
